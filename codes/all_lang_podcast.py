...
#     md_files = [
#         r"C:\Users\Maham Jafri\Documents\Office Tasks\Durbeen\ResearchArticles\research_mds\Deficiency, Contamination, and the Signal Processing Metaphor.md",
#     ]
#     podcast = Podcast()
#     path = podcast.generate_podcast(language="urdu", md_file_paths=md_files)
#     print(path)


# IN THE BELOW CODE, WE ARE GENERATING A PODCAST BY INSTRUCTING THE TTS PART TO SPEAK IN THE LANGUAGE SPECIFIED BY THE USER
# import os
# import json
# import re
# import time
...
#         return """
# You are an experienced podcast host creating an engaging conversation between Sascha and Marina in English about the provided article.
...
#   {"speaker": "Marina", "text": "I'm doing great, Sascha! What’s the article about?"}
# ]
# The conversation should be natural, engaging, at least 30,000 characters long, and contain filler words, excitement, and short spoken sentences.
# """
...
#         print(f"[TIMER] extract_text_from_md_files took: {elapsed:.2f}s")
#         return full_text.strip()
...
#             return None
#         elapsed = time.time() - start
#         print(f"[TIMER] generate_conversation took: {elapsed:.2f}s")
#         return conversation

#     def synthesize_speech_openai(self, text, speaker, index, language):
#         start = time.time()
#         voice_id = self.speaker_voice_map.get(speaker, "onyx")
#         client = openai.OpenAI(api_key=self.api_key)
...
#             voice=voice_id,
#             input=text,
#             instructions=f"Speak in {language}",
#         )
#         os.makedirs("audio-files", exist_ok=True)
#         filename = f"audio-files/{index:04d}_{speaker}.mp3"
#         with open(filename, "wb") as f:
...
#         elapsed = time.time() - start
#         print(
#             f"[TIMER] synthesize_speech_openai for index {index} took: {elapsed:.2f}s"
#         )
...
#         final_clip.write_audiofile(output_file, codec="libmp3lame")
#         elapsed = time.time() - start
#         print(f"[TIMER] merge_audios took: {elapsed:.2f}s")
...
#         for index, part in enumerate(conversation):
#             self.synthesize_speech_openai(
#                 part["text"], part["speaker"], index, language
#             )
...
#         self.merge_audios("audio-files", output_path)
#         elapsed = time.time() - start
#         print(f"[TIMER] generate_audio took: {elapsed:.2f}s")
#         return f"/static/audio/{output_filename}"
...
#         outputfilepath = self.generate_audio(conversation, language, md_file_paths)
#         elapsed = time.time() - start
#         print(f"[TIMER] generate_podcast took: {elapsed:.2f}s")
#         return outputfilepath


# THE BELOW CODE IS THE IMPLEMENATION FOR THE PODCAST GENERATION BY FIRST TRANSLATING THE ENGLISH SCRIPT TO THE SPOKEN LANGUAGE AND THEN GENERATING AUDIO FOR EACH PART OF THE CONVERSATION.
# import os
# import json
# import re
# import openai
...
#     def translate_conversation(self, conversation, language):
#         translated = []
#         for part in conversation:
#             translated_text = self.translate_text(part["text"], language)
#             translated.append({"speaker": part["speaker"], "text": translated_text})
#         return translated
...
#         # Translate English script to spoken language
#         translated_conversation = self.translate_conversation(conversation, language)

#         for index, part in enumerate(translated_conversation):
#             self.synthesize_speech_openai(part["text"], part["speaker"], index)
